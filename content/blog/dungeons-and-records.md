...
1) The note's original image data would need to be encoded as a Base64 (link) string. By doing so, the `src=` attribute would directly contain all of the image data, without relying on an external file. Notion supports directly embedding image data like this, but not referencing external files for the data.
...
- Optionally, also use the OpenAI API to translate image data. This allows for a comparison of existing OCR technology, versus state-of-the-art multi-modal LLM tech
